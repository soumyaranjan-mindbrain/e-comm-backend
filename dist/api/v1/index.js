"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = __importDefault(require("express"));
const health_1 = __importDefault(require("./health"));
const auth_1 = __importDefault(require("./auth"));
const profile_1 = __importDefault(require("./profile"));
const categories_1 = __importDefault(require("./categories"));
const products_1 = __importDefault(require("./products"));
const product_registers_1 = __importDefault(require("./product-registers"));
const product_image_registers_1 = __importDefault(require("./product-image-registers"));
const product_ratings_1 = __importDefault(require("./product-ratings"));
const user_addresses_1 = __importDefault(require("./user-addresses"));
const coupon_codes_1 = __importDefault(require("./coupon-codes"));
const cart_1 = __importDefault(require("./cart"));
const order_1 = __importDefault(require("./order/order"));
const OrderReturnRouter_1 = __importDefault(require("./return/OrderReturnRouter"));
const WalletRouter_1 = __importDefault(require("./wallet/WalletRouter"));
const AdminWalletRouter_1 = __importDefault(require("./wallet/AdminWalletRouter"));
const NotificationRouter_1 = __importDefault(require("./notifications/NotificationRouter"));
const FaqRouter_1 = __importDefault(require("./faq/FaqRouter"));
const PolicyRouter_1 = __importDefault(require("./policies/PolicyRouter"));
const ContactRouter_1 = __importDefault(require("./contact/ContactRouter"));
const v1 = express_1.default.Router();
v1.use("/health", health_1.default);
v1.use("/auth", auth_1.default);
v1.use("/profile", profile_1.default);
v1.use("/categories", categories_1.default);
v1.use("/products", products_1.default);
v1.use("/product-register", product_registers_1.default);
v1.use("/product-image-register", product_image_registers_1.default);
v1.use("/product-ratings", product_ratings_1.default);
v1.use("/user-addresses", user_addresses_1.default);
v1.use("/coupon-codes", coupon_codes_1.default);
v1.use("/cart", cart_1.default);
v1.use("/orders", order_1.default);
v1.use("/order-returns", OrderReturnRouter_1.default);
v1.use("/wallet", WalletRouter_1.default);
v1.use("/admin/wallet", AdminWalletRouter_1.default);
v1.use("/notifications", NotificationRouter_1.default);
v1.use("/faq", FaqRouter_1.default);
v1.use("/policies", PolicyRouter_1.default);
v1.use("/contact", ContactRouter_1.default);
exports.default = v1;
