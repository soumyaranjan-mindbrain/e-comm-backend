...
class OrderRepository {
    ...
    async createOrder(data) {
        for (let attempt = 0; attempt < OrderRepository.ORDER_ID_RETRY_LIMIT; attempt++) {
    ...
    async getShopStockItemsByProductIds(productIds) {
        if (productIds.length === 0)
            return [];
        return prisma_client_1.default.shopStockItem.findMany({
            where: {
                itemId: {
                    in: productIds,
                },
