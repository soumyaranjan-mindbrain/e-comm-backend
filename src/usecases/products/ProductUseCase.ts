...
const transformProduct = (product: any) => {
    ...
    const totalQty = stockItems.reduce((acc: number, item: any) => acc + (item.curQty || 0), 0);

    return {
        ...product,
        price: primaryStock?.saleRate || null,
        mrp: primaryStock?.mrpRate ? parseFloat(primaryStock.mrpRate) : (primaryStock?.saleRate || null),
        totalStock: totalQty,
        inStock: totalQty > 0,
        variants: variants,
